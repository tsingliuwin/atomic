...
## 📁 Project Structure
- **[`ATOMIC_THEORY.md`](./ATOMIC_THEORY.md)**: **The Manifesto.** The definitive source of truth for all laws, protocols, and philosophical underpinnings.
- **[`ATOMIC_METADATA.json`](./ATOMIC_METADATA.json)**: **The Gene Registry Template.** The standardized schema for tracking atom identity, types, and evolutionary history.
- **[`ATOMIC_THEORY_DISCUSSION_LOG.md`](./ATOMIC_THEORY_DISCUSSION_LOG.md)**: **The History of Intent.** A record of the evolutionary logic and debates that shaped the theory.

## 🛠 How to Use
1. **Understand the Laws**: Read the "DNA Laws" in `ATOMIC_THEORY.md`.
2. **Initialize Your Project**: Copy the `ATOMIC_METADATA.json` template to your sub-project.
3. **Empower Your AI**: Provide these documents to your AI agent (like Gemini CLI) to enable autonomous atomic development.
4. **Follow the Protocol**: Ensure every commit follows the `[Atom:<id>]` semantic format to maintain the gene sequence.
...
*Atomic Theory is licensed under the MIT License - see the [LICENSE](LICENSE) file for details (coming soon).*
